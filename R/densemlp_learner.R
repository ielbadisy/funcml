...
# architecture options (residual connections, gated blocks, feature
# interactions, moving-average weights, learning-rate schedules, internal
# ensembles) implemented natively in C++ via RcppArmadillo (no
# `torch`/`libtorch` dependency), and is wired in here as an additional
# learner rather than a replacement.

.densemlp_fit <- function(X, y, spec, task, levels, ...) {
  assert_package("densemlp", "densemlp")

  densemlp_task <- if (identical(task, "regression")) {
    "regression"
